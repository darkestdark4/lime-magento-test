...
class Save extends Slider
{
    ...
    if (isset($data['from_date']) && $data['from_date']) {
        $dateFilter = $this->_dateFilter ?? \Magento\Framework\App\ObjectManager::getInstance()
            ->get(\Magento\Framework\Stdlib\DateTime\Filter\Date::class);

        $data['from_date'] = $dateFilter->filter($data['from_date']);
    }
